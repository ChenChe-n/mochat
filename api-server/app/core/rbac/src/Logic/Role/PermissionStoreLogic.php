 ...
 */
namespace MoChat\App\Rbac\Logic\Role;
...
class PermissionStoreLogic
{
    ...
    public function handle(int $roleId, array $menuIds): bool
    {
        ...
        Db::beginTransaction();
        try {
            empty($dbData['add']) || $this->roleMenuService->createRbacRoleMenus($dbData['add']);
            empty($dbData['del']) || $this->roleMenuService->deleteRbacRoleMenus($dbData['del']);
    ...
    protected function diffRoleMenu(int $roleId, array $menuIds): array
    {
        $res = [];
        ## 原菜单
        $oldMenuIds = $this->roleMenuService->getRbacRoleMenusByRoleId([$roleId], ['id', 'menu_id']);
